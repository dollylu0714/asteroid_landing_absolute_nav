...
for k = 1:len
    ...
    du_c_p = 0.5; % pixel
    du_c_m = 0.003; % m
    [Z_real,feature_a,n] = abs_feature_camera(X(k+1,:),du_c_m); %模拟真实观测  X 探测器的真实状态, Xp 特征的状态
    Rk  = abs_feature_cov(feature_a,du_c_m,n);% 计算观测协方差;
    
    Hk = JacobianH_camera(Xk1,feature_a,n); % 计算观测雅可比矩阵
    Z_est = abs_feature_est(Xk1,feature_a,n); % 计算预测观测量
...
for kk = 1:len
    ...
    % 模拟观测量
    du_c_m = 0.003; % m
    [Z_real,feature_a,n] = abs_feature_camera(X(kk+1,:),du_c_m);
    Rk  = abs_feature_cov(feature_a,du_c_m,n);% 计算观测协方差;
    ...
    for k=1:num2
        %         Z1(:,k)=f(X(:,k));
        Z1(:,k) = abs_feature_est(X1(:,k)',feature_a,n);
        z1=z1+Wm(k)*Z1(:,k);
    end
