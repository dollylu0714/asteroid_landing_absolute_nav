function [Z_real,feature_a,feature_num_in_FOV] = abs_fusion( X_orbit,dp)
...
%%
Z_real = zeros(feature_num_in_FOV*2+1,1);
for i = 1:feature_num_in_FOV
    a1 = X_orbit(1)-feature_a(i,1);   b1 = X_orbit(2) - feature_a(i,2) ; c1 = X_orbit(3) - feature_a(i,3);
    Z_real((i-1)*2+1,1) = f*a1/c1 + dp(1)*rand(1,1);
    Z_real((i-1)*2+2,1) = f*b1/c1 + dp(1)*rand(1,1);
end
Z_real(feature_num_in_FOV*2+1,1) = X_orbit(3)+dp(2)*rand(1,1);
